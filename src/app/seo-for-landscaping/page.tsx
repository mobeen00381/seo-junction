    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 font-inter text-left">
      ...
      <section style={{background:'linear-gradient(135deg,#065F46,#064E3B)'}} className="px-6 py-40 text-center text-white">
        <div className="max-w-3xl mx-auto">
          <div className="inline-block text-[10px] font-black tracking-[2px] uppercase px-4 py-1.5 rounded-full mb-6 border" style={{background:'rgba(255,255,255,0.1)',borderColor:'rgba(255,255,255,0.2)',color:'#A7F3D0'}}>🌳 Green Growth SEO</div>
          <h1 className="text-4xl md:text-6xl font-extrabold mb-6 leading-tight tracking-tight text-center">
            Transform Your <br /><span className="text-emerald-300">Landscaping Leads</span> — 100% Autopilot
          </h1>
      ...
      <section className="py-24 px-6 bg-gray-50 dark:bg-slate-900/50">
        <div className="max-w-4xl mx-auto text-left">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              {icon:'🏡',title:'Seasonal Yard Care',desc:'We target high-intent searches like "mulching near me", "lawn aeration", and "fall cleanup" to drive local homeowner calls year-round.'},
              {icon:'🏗️',title:'Hardscaping Leads',desc:'Our AI automatically posts about "paver patios", "retaining walls", and "fire pits", capturing high-value construction cases.'},
              {icon:'🗺️',title:'Map Pack Presence',desc:'Landscaping is a local game. We ensure your hours, location, and service categories are hyper-optimized for Map Pack results.'},
            ].map((p,i)=>(
      ...
      <section className="py-20 px-6 bg-white dark:bg-slate-950 border-t border-gray-100 dark:border-slate-800">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-xl text-emerald-600 dark:text-emerald-400 mb-10 text-center uppercase tracking-widest font-bold font-inter">Get Your Site Live For Only <span className="text-gray-900 dark:text-white">$19 One-Time</span></p>
          <Link href="/onboarding" className="inline-block font-bold text-white px-12 py-5 rounded-2xl shadow-2xl transition-all hover:scale-105" style={{background:'var(--premium-gradient)'}}>Start Free Trial — Built for Landscapers →</Link>
